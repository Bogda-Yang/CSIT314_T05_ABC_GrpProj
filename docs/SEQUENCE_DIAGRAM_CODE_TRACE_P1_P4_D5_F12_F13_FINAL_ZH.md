# Sequence Diagram 与 BCE Mapping 最终对照版：P1、P4、D5、F12、F13

本文件是在 `SEQUENCE_DIAGRAM_CODE_TRACE_P1_P4_D5_F12_F13_ZH.md` 的基础上整理出的最终版。它额外加入了五个 User Story 在对应 `BCE_MAPPING.md` 文档中的 BCE class、class definition 和 method definition 内容，方便同时对照：

- Sequence Diagram 每一步流程
- `BCE_MAPPING.md` 中列出的 Boundary / Controller / Entity
- 真实代码文件和行号
- 需要注意的 route handler、UI action、inline logic、wrapper method 等细节

## P1 Create Category

### BCE_MAPPING 对应部分

来源：`docs/P1-7_BCE_MAPPING.md:19`
...
- BCE_MAPPING 中 P1 的 Boundary、Controller、Entity 和真实代码能对应上。

## P4 View All Categories

### BCE_MAPPING 对应部分

来源：`docs/P1-7_BCE_MAPPING.md:43`
...
- BCE_MAPPING 中额外列出的 `SearchCategories()` 和 `FilterCategories()` 都是真实 wrapper 方法；不过 P4 主列表流程优先走 `GetCategoryList()`，其中 search 分支会调用 `SearchCategories()`，status filter 主流程是 inline list comprehension。

## D5 Save Campaign to Favourite List

### BCE_MAPPING 对应部分

来源：`docs/D1-11_BCE_MAPPING.md:56`
...
- BCE_MAPPING 中 D5 的 Boundary、Controller、Entity 都能在 Sequence Diagram 的主流程中找到对应调用。

## F12 View Completed Campaigns

### BCE_MAPPING 对应部分

来源：`docs/F1-13_BCE_MAPPING.md:128`
...
- BCE_MAPPING 中的 `ViewCompletedCampaignDetails()`、`GetCompletedCampaignDetails()`、`CompletedCampaignRecord.GetCampaignById()` 是 completed campaign detail path；当前 Sequence Diagram 主要展示 list path，所以这些方法属于同一 user story 的详情补充流程。

## F13 Filter Campaigns Using Selectable Criteria

### BCE_MAPPING 对应部分

来源：`docs/F1-13_BCE_MAPPING.md:137`
...
- BCE_MAPPING 中列出的 `CampaignFilterController.FilterCampaigns()` 也是真实方法，但当前 Sequence Diagram 主流程使用的是 `RetrieveFilteredCampaignResults()`，两者都属于 F13 的 Controller 层过滤能力。

## 总体建议修改的 Sequence Diagram 表达
...
| User Story | BCE_MAPPING 与代码 | Sequence Diagram 与代码 | 主要注意点 |
|---|---|---|---|
| P1 | 对应完整 | 主流程对应完整 | 真实代码有 Boundary 和 Controller 双重验证。 |
| P4 | 对应完整 | 主流程对应，但入口应理解为 dashboard route | `Display categories` 和 status filter 不是独立方法。 |
| D5 | 对应完整 | 主流程对应完整 | 点击收藏是 UI action，不是后端方法。 |
| F12 | 对应完整 | list 主流程对应完整 | detail 相关方法在 BCE_MAPPING 中列出，但当前 Sequence Diagram 主要展示 list path。 |
| F13 | 对应完整 | 主流程对应完整 | `Clear filter controls` 是默认 query 参数行为，不是单独方法。 |
